...
for z=1:size(imgser,3)
    G(:,:,z) =fftshift(fftshift(fft2(double(imgser(:,:,z))).*conj(fft2(double(imgser(:,:,z)))),1),2);
    %G(:,:,z) =(fft2(double(imgser(:,:,z))).*conj(fft2(double(imgser(:,:,z)))));
     %G(:,:,z) = (((real(ifft2(fft2(double(imgser(:,:,z))).*conj(fft2(double(imgser(:,:,z))))))))/(mean(mean(imgser(:,:,z)))^2*size(imgser,1)*size(imgser,2))) - 1;
     if ishandle(h)
