function [a] = flowfitGUI(ax,ax2,time,corr,PSFSize,weights);
...
[a,resnorm,residual] = lsqcurvefit(@flow,a0,time,corr.*weights,lb,ub,curvefitoptions,weights);
 ypred = flow(a,timeAvg,ones(size(timeAvg)));
    ...
    title(ax,'Single Component Flow Fit','FontSize',20)
    %set(ax,'tight')
    xlim=get(ax,'XLim');
    ylim=get(ax,'YLim');
     text(ax,0.1*xlim(2),0.7*ylim(2),['|v| =' num2str((PSFSize)/(a(2))) ' \mum s^{-1}'],'Color', [1 0 0],'FontSize',20)
    text(ax,0.1*xlim(2),0.8*ylim(2),['<N_{f}> =' num2str(1/((a(1)*(pi)*(PSFSize^2)))) ' particles \mum^{-2}'],'Color', [1 0 0],'FontSize',20)
