function [postConv] = simul8tr2GUI(ax,sizeXdesired,sizeYdesired,sizeT,density,bleachType,bleachDecay,qYield,pixelsize,timesize,PSFType,PSFSize,PSFZ,noBits,diffCoeff,flowX,flowY,flowZ,countingNoise,backgroundNoise,FCS,masksize);
...
th = text(ax,1,1,'Simulating data...0%','VerticalAlignment','bottom','HorizontalAlignment','right');
 FCSCounter = 0;
for i = 1:sizeT;
...
%         %Bleaches stuff
    ...
    if FCS==0
    ph.XData = [0 i/sizeT i/sizeT 0];
    th.String = sprintf('%Simulating data....0f%%',round(i/sizeT*100));
...
% mask data by fraction defined by maskdata if wanting to demonstrate the
% noise correction
if masksize>0
    [mask]=makecentralmask(sizeX,masksize);
    postConv=postConv.*repmat(mask,[1 1 sizeT]);
end
