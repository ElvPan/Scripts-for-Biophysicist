function [timecorr] = stics_asgui(imgser,upperTauLimit);
...
% pair is the nth pair of a lag time
       ...
       SeriesMean = squeeze(mean(mean(mean(imgser))));
       ...
       for tau = 0:upperTauLimit-1
           ...
           for pair=1:(size(imgser,3)-tau)
              % lagcorr(:,:,pair) = (fftshift(real(ifft2(fft2(imgser(:,:,pair)).*conj(fft2(imgser(:,:,(pair+tau)))))))/(SeriesMean(pair)*SeriesMean(pair+tau).*size(imgser,1).*size(imgser,2)))-1;
               lagcorr(:,:,pair) = fftshift(real(ifft2(fft2(imgser(:,:,pair)).*conj(fft2(imgser(:,:,(pair+tau)))))))/(SeriesMean^2);
