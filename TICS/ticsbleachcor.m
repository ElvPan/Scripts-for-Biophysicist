function [GtAvg,RawGt,RawGtBleCor] = ticsbleachcor(imgser,timesize,k,T);
...
% TAKING INTO ACCOUNT THE BLEACHING DECAY RATE k
...
% pair is the nth pair of a lag time
       ...
       RawGtBleCor = zeros(mean(1:size(imgser,3))*(size(imgser,3)),2) - 1;
       ...
       for tau = 0:(size(imgser,3)-1)
           ...
           RawGtBleCor(tau,:)=(k*(T-tau)/(exp(k*(T-tau))-1))*RawGt(tau,:);
