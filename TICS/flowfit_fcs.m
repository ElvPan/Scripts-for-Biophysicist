function [a] = flowfit_fcs(time,corr,PSFSize,PSFZ,weights);
...
% 
    ...
    xlim=get(gca,'XLim');
    ylim=get(gca,'YLim');
    text(0.1*xlim(2),0.7*ylim(2),['|v| =' num2str((PSFSize)/(a(2))) ' \mum s^{-1}'],'Color', [1 0 0])
    text(0.1*xlim(2),0.8*ylim(2),['<N_{f}> =' num2str(1/((a(1)*(pi^1.5)*(PSFZ*PSFSize^2)))) ' particles \mum^{-2}'],'Color', [1 0 0])
