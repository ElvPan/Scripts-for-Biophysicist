function [GtAvg] = ticcs(imgser1,imgser2,timesize);
...
% pair is the nth pair of a lag time
       ...
       GtAvg = zeros(size(imgser1,3),2);
       lagcorr=zeros(1,1,size(imgser1,3));
       %imgser = double(imgser);
       index = 1;
       %Calculates mean of each image before to save time in calculation
       imgSerMean1 = squeeze(mean(mean(imgser1,2)));
       imgSerMean2 = squeeze(mean(mean(imgser2,2)));
       for tau = 0:(size(imgser1,3)-1)

           lagcorr = zeros(1,1,(size(imgser1,3)-tau)); % preallocation
           %lagcorrStd = zeros(1,1,(size(imgser,3)-tau)); % preallocation
           for pair=1:(size(imgser1,3)-tau)
               corrNonAvgNonNorm = (double(imgser1(:,:,pair)).*double(imgser2(:,:,pair+tau)));
               corrNonAvg = corrNonAvgNonNorm/(imgSerMean1(pair)*imgSerMean2(pair+tau));
