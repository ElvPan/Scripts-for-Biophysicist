function [a] = difffit(time,corr,PSFSize,weights);
...
figure
    ...
    title('Single Component Diffusion Fit','FontSize',10)
    axis tight
    xlim=get(gca,'XLim');
    ylim=get(gca,'YLim');
    text(0.2*xlim(2),0.6*ylim(2),['D =' num2str((PSFSize^2)/(4*a(2))) ' \mum^{2} s^{-1}'],'Color', [1 0 0])
    text(0.2*xlim(2),0.8*ylim(2),['<N> =' num2str(1/(a(1)*pi*(PSFSize^2))) ' particles \mum^{-2}'],'Color', [1 0 0])
    subplot(3,1,[3])
    hold on
plot(timeAvg,0,'-k')
